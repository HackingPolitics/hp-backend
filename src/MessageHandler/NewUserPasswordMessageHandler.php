...
class NewUserPasswordMessageHandler implements MessageHandlerInterface, ServiceSubscriberInterface
{
     ...
     */
    public function __invoke(NewUserPasswordMessage $message)
    {
        $this->logger()->debug(
            'Processing NewUserPasswordMessage for user '
            .$message->userId
        );
        ...
        if ($sent) {
            $this->logger()
                ->info("Sent new password validation email to user {$user->getEmail()}!");
        } else {
            $this->logger()
                ->error("Failed to send the new password validation email to {$user->getEmail()}!");
        }
    }
    ...
    private function sendValidationMail(Validation $validation, string $url): bool
    {
        ...
        $email = (new TemplatedEmail())
            ...
            ->htmlTemplate('security/mail.new-password.html.twig')
            ->context([
                'id'            => $validation->getId(),
