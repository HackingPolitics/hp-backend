...
class UsedActionMandateVoter extends Voter
{
    /**
     * {@inheritdoc}
     */
    protected function supports($attribute, $subject): bool
    {
        return in_array($attribute, ['CREATE', 'DELETE'])
            && $subject instanceof UsedActionMandate;
    }

    /**
     * {@inheritdoc}
     *
     * @param UsedActionMandate $subject
     */
    protected function voteOnAttribute(string $attribute, $subject, TokenInterface $token): bool
    {
        ...
        $actionMandate = $subject->getActionMandate();
        if (!$actionMandate) {
            // on creation handled by validator, else it's an error
            return 'CREATE' === $attribute;
        }

        $project = $actionMandate->getProject();
        if (!$project) {
            return false;
        }
