...
class NegationDenormalizer implements ContextAwareDenormalizerInterface, DenormalizerAwareInterface
{
    use DenormalizerAwareTrait;

    private const ALREADY_CALLED = 'NEGATION_DENORMALIZER_ALREADY_CALLED';
    ...
    public function denormalize($data, $type, $format = null, array $context = [])
    {
        ...
        if ($project && $token && $token->getUser() instanceof UserInterface) {
            ...
            if (ProjectMembership::ROLE_COORDINATOR === $project->getUserRole($currentUser)) {
                $context['groups'][] = 'negation:coordinator-write';

                // this denormalizer is never called for the creation of
                // negations so we can simply add this here without checking
                // the context for operation_type=item & item_operation_name=put
                $context['groups'][] = 'negation:coordinator-update';
            }

            // writers & coordinators
            if ($project->userCanWrite($currentUser)) {
                $context['groups'][] = 'negation:member-write';

                // same as before, no additional checks needed
                $context['groups'][] = 'negation:member-update';
            }
        }
