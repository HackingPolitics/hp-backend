...
class CounterArgumentNormalizer implements ContextAwareNormalizerInterface, NormalizerAwareInterface
{
    use NormalizerAwareTrait;

    private const ALREADY_CALLED = 'COUNTER_ARGUMENT_NORMALIZER_ALREADY_CALLED';
        ...
        if ($token && $token->getUser() instanceof UserInterface) {
            ...
            if ($project) {
                if ($project->userCanRead($currentUser)) {
                    $context['groups'][] = 'counterArgument:member-read';
                }

                $role = $project->getUserRole($currentUser);
                if (ProjectMembership::ROLE_COORDINATOR === $role) {
                    $context['groups'][] = 'counterArgument:coordinator-read';
                }

                if (ProjectMembership::ROLE_OBSERVER === $role) {
                    $context['groups'][] = 'counterArgument:observer-read';
                }

                if (ProjectMembership::ROLE_WRITER === $role) {
                    $context['groups'][] = 'counterArgument:writer-read';
                }
            }
        }
    ...
    public function supportsNormalization($data, $format = null, array $context = []): bool
    {
        ...
        return $data instanceof CounterArgument;
    }
}
