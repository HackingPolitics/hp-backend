...
class ActionMandateNormalizer implements ContextAwareNormalizerInterface, NormalizerAwareInterface
{
    use NormalizerAwareTrait;

    private const ALREADY_CALLED = 'ACTION_MANDATE_NORMALIZER_ALREADY_CALLED';
        ...
        if ($token && $token->getUser() instanceof UserInterface) {
            ...
            if ($project) {
                if ($project->userCanRead($currentUser)) {
                    $context['groups'][] = 'actionMandate:member-read';
                }

                $role = $project->getUserRole($currentUser);
                if (ProjectMembership::ROLE_COORDINATOR === $role) {
                    $context['groups'][] = 'actionMandate:coordinator-read';
                }

                if (ProjectMembership::ROLE_OBSERVER === $role) {
                    $context['groups'][] = 'actionMandate:observer-read';
                }

                if (ProjectMembership::ROLE_WRITER === $role) {
                    $context['groups'][] = 'actionMandate:writer-read';
                }
            }
        }
    ...
    public function supportsNormalization($data, $format = null, array $context = []): bool
    {
        ...
        return $data instanceof ActionMandate;
    }
}
